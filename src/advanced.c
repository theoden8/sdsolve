...
attributes:;
  sd_t *s = malloc(sizeof(sd_t));
  assert(s != NULL);
  s->n = n;
  s->ne2 = n*n;
  s->ne4 = s->ne2 * s->ne2;
  s->table = malloc(sizeof(val_t) * s->ne4);
  assert(s->table != NULL);
...
row:;
  val_t mem[s->w];
  memset(mem, 0, s->w * sizeof(val_t));
  s->r = malloc(sizeof(sz_t) * s->w * s->ne2);
  assert(s->r != NULL);
...
static void dump_covererd(const cov_t *pair, sz_t len1, sz_t len2) {
  puts("covered");
  printf("row: ");
  for(sz_t i=0;i<len1;++i)printf("%hhd ", pair->row[i]);putchar('\n');
  printf("col: ");
  for(sz_t i=0;i<len2;++i)printf("%hhd ", pair->col[i]);putchar('\n');
}

static void dump_choice(const sol_t *pair, sz_t len1, sz_t len2) {
  puts("choice");
  printf("row: ");
  for(sz_t i=0;i<len1;++i)printf("%d ", pair->row[i]);putchar('\n');
  printf("col: ");
  for(sz_t i=0;i<len2;++i)printf("%d ", pair->col[i]);putchar('\n');
}
...
static inline min_t sd_update(const sd_t *s, cov_t *cov, sz_t r, ACTION flag) {
  ...
  for(sz_t c = 0; c < NO_CONSTR; ++c)cov->col[s->c[CIDX(r, c)]] ^= LBIT;
  for(sz_t c = 0; c < NO_CONSTR; ++c)
...
static inline void sd_forward(const sd_t *s, cov_t *cov, sz_t r, sz_t c, min_t *m) {
  assert(c < NO_CONSTR);
  sz_t cidx = s->c[CIDX(r, c)];
  assert(cidx < s->w);
  for(sz_t ir = 0; ir < s->ne2; ++ir) {
    sz_t rr = s->r[RIDX(cidx, ir)];
    assert(rr < s->h);
    if(cov->row[rr]++ != 0)continue;
    for(sz_t ic = 0; ic < NO_CONSTR; ++ic) {
      sz_t cc = s->c[CIDX(rr, ic)];
      assert(cidx < s->w);
      --cov->col[cc];
      if(cov->col[cc] < m->min)
        m->min = cov->col[cc],
        m->fail_rate = cov->colfail[cc],
        m->min_col = cc;
    }
  }
}
...
static void dump_sd(sd_t *s, sol_t soln, int i) {
  ...
  for(int j = 0; j < i; ++j) {
    int r = s->r[RIDX(soln.col[j], soln.row[j])];
  ...
  while(1) {
    while(i >= 0 && i < s->ne4 - no_hints) {
      ...
      if(action == BACKTRACK && cr != UNDEF_SIZE)
        sd_update(s, &cov, s->r[RIDX(cc, cr)], BACKTRACK);
      sz_t ir = (cr == UNDEF_SIZE) ? 0 : cr + 1;
      while(ir < s->ne2) {
        if(cov.row[s->r[RIDX(cc, ir)]] == ROWCOL)break;
        ++ir;
      }
      if(ir < s->ne2)
        action = FORWARD,
        m = sd_update(s, &cov, s->r[RIDX(cc, ir)], FORWARD),
        soln.row[i==-1?0:i] = ir,
        ++i;
      else {
        ++cov.colfail[m.min_col],
